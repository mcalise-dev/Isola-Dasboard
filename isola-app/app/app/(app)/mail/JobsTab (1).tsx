...
export default function JobsTab() {
  ...
  const [punchOpen, setPunchOpen] = useState<Record<string, number>>({});
  const [pForm, setPForm] = useState<any>({ item: "", priority: "normal", due_date: "", raised_by: "" });
  const [pBusy, setPBusy] = useState(false);

  async function load() {
    const [{ data }, { data: costRows }, { data: punchRows }] = await Promise.all([
      ...
      supabase.from("punch_list").select("job_id,done").eq("done", false),
    ]);
    const pm: Record<string, number> = {};
    (punchRows ?? []).forEach((r: any) => { if (r.job_id) pm[r.job_id] = (pm[r.job_id] ?? 0) + 1; });
    setPunchOpen(pm);
  ...
  function setStatus(j: Job, status: string) {
    if (status === "complete" && j.status !== "complete") {
      const openItems = punch.filter((t: any) => !t.done);
      if (openItems.length && !confirm(
        `${openItems.length} punch item${openItems.length === 1 ? "" : "s"} still open on this job:\n\n` +
        openItems.slice(0, 6).map((t: any) => "• " + t.item).join("\n") +
        (openItems.length > 6 ? `\n• …and ${openItems.length - 6} more` : "") +
  ...
  useEffect(() => {
    setDraft({});
    setPForm({ item: "", priority: "normal", due_date: "", raised_by: "" });
    if (!viewing) { setPhotos([]); setLabor([]); setJobbook(null); setJtasks([]); setJcosts([]); setPunch([]); return; }
    ...
    supabase.from("punch_list").select("id,item,raised_by,priority,due_date,done,done_at,notes,created_at").eq("job_id", viewing.id).order("done").order("created_at", { ascending: false }).then(({ data }: any) => setPunch(data ?? []));
  }, [viewing?.id]);
  ...
  async function reloadPunch() {
    const { data } = await supabase.from("punch_list").select("id,item,raised_by,priority,due_date,done,done_at,notes,created_at").eq("job_id", viewing!.id).order("done").order("created_at", { ascending: false });
    setPunch(data ?? []);
    const { data: open } = await supabase.from("punch_list").select("job_id,done").eq("done", false);
    const pm: Record<string, number> = {};
    (open ?? []).forEach((r: any) => { if (r.job_id) pm[r.job_id] = (pm[r.job_id] ?? 0) + 1; });
    setPunchOpen(pm);
  }

  async function addPunch() {
    if (!pForm.item.trim()) return;
    setPBusy(true);
    const { error } = await supabase.from("punch_list").insert({
      job_id: viewing!.id,
      item: pForm.item.trim(),
      priority: pForm.priority,
      due_date: pForm.due_date || null,
      raised_by: pForm.raised_by.trim() || null,
    });
    setPBusy(false);
    if (error) { alert("Save failed: " + error.message); return; }
    setPForm({ item: "", priority: "normal", due_date: "", raised_by: "" });
  ...
  async function togglePunch(t: any) {
    const done = !t.done;
    setPunch(punch.map((x) => (x.id === t.id ? { ...x, done, done_at: done ? new Date().toISOString() : null } : x)));
    setPunchOpen({ ...punchOpen, [viewing!.id]: Math.max(0, (punchOpen[viewing!.id] ?? 0) + (done ? -1 : 1)) });
    const { error } = await supabase.from("punch_list").update({ done, done_at: done ? new Date().toISOString() : null }).eq("id", t.id);
    if (error) { alert("Save failed: " + error.message); reloadPunch(); }
  }

  async function removePunch(t: any) {
          ...
          <button key={j.id} onClick={() => setViewing(j)}
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="shrink-0 text-right">
                ...
                {punchOpen[j.id] ? (
                  <span className="block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full border border-amber-500/50 bg-amber-500/10 text-amber-300">
                    {punchOpen[j.id]} punch
                  </span>
                ) : null}
              </div>
            </div>
              ...
              <div className="px-5 py-4 space-y-4">
                ...
                <div>
                  {(() => {
                    ...
                    const open = punch.filter((t: any) => !t.done);
                    const overdue = open.filter((t: any) => t.due_date && t.due_date < today).length;
                    return (
                      <>
                            ...
                            {punch.map((t: any) => {
                              ...
                              return (
                                <div key={t.id} className={`flex items-start gap-2.5 rounded-xl border bg-neutral-950 px-3 py-2 ${late ? "border-red-500/50" : t.priority === "high" && !t.done ? "border-amber-500/40" : "border-neutral-800"}`}>
                                  ...
                                  <button onClick={() => removePunch(t)} className="shrink-0 text-neutral-600 hover:text-red-400 text-xs">✕</button>
                                </div>
                              );
                            })}
                          </div>
                        ...
                        <div className="space-y-2">
                          <input className={input} placeholder="What needs fixing before this closes out?" value={pForm.item}
                            onChange={(e) => setPForm({ ...pForm, item: e.target.value })}
                            onKeyDown={(e) => { if (e.key === "Enter") addPunch(); }} />
                          <div className="grid grid-cols-2 gap-2">
                            <input className={input} placeholder="Raised by (optional)" value={pForm.raised_by} onChange={(e) => setPForm({ ...pForm, raised_by: e.target.value })} />
                            <input className={input} type="date" value={pForm.due_date} onChange={(e) => setPForm({ ...pForm, due_date: e.target.value })} />
                          </div>
                          <div className="flex gap-2">
                            {(["normal", "high"] as const).map((lvl) => (
                              <button key={lvl} type="button" onClick={() => setPForm({ ...pForm, priority: lvl })}
                                className={`flex-1 rounded-lg border py-2 text-xs font-semibold ${pForm.priority === lvl ? (lvl === "high" ? "border-amber-500/60 text-amber-300 bg-neutral-800" : "border-neutral-300 text-white bg-neutral-800") : "border-neutral-700 text-neutral-500"}`}>
                                {lvl === "high" ? "! Priority" : "Normal"}
                              </button>
                            ))}
                            <button onClick={addPunch} disabled={pBusy} className="shrink-0 rounded-lg bg-white text-neutral-900 px-4 text-xs font-bold disabled:opacity-60">{pBusy ? "…" : "+ Add"}</button>
                          </div>
                        </div>
                        <p className="text-[10px] text-neutral-600 mt-1.5">Open items show on the job card and you get a warning before this job can be marked complete.</p>
                      </>
                    );
                  })()}
                </div>
